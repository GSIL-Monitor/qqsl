...
*/
	...
	public void testListToStringPoing() {
		...
		for (int i = 0; i <= 87; i++) {
			...
			graph.setBaseType(Config.CommonType.SD);
			list.add(graph);
		}
		String str1 = "point";
...
//				row2 - the row (0 based) of the second cell.
	...
	public void testBaseType(){
		...
		for (int i = 0; i < CommonAttributes.BASETYPEE.length; i++) {
			if (!Config.CommonType.valueOf(i).toString().equals(CommonAttributes.BASETYPEE[i])) {
				System.out.println(Config.CommonType.valueOf(i)+"==>>"+CommonAttributes.BASETYPEE[i]);
			}
...
//		Map<Config.CommonType, List<Build>> buildMap = testFindByProject();
...
//		os.close();
	...
	public Map<Config.CommonType,List<Build>> testFindByProject(){
		...
		Map<Config.CommonType,List<Build>> builds = new LinkedHashMap();
		List<Build> b = new ArrayList<>();
//		List<Build> builds1 = new ArrayList<>();
//		builds1.add(new Build());
//		builds.put(Config.CommonType.QS, builds1);
...
//		Map<Config.CommonType,List<Build>>map=fieldService.groupBuild(list);
//		return map;
		return null;
	}
	...
	public void testOutputAllModel() throws IOException {
		...
		Map<Config.CommonType,List<Build>>map=fieldService.groupBuild(buildsDynamic);
		Row row = null;
		Cell cell = null;
		boolean flag;
