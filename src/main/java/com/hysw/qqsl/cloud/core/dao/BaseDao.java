...
import com.hysw.qqsl.cloud.core.entity.Filter.Operator;
...
//		Order aaa = criteriaBuilder.desc(root.get(filters.get(0).getProperty()));
//		criteriaQuery.orderBy(aaa);
...
//	}
	...
	public List<T> findList(Filter filter) {
		...
		criteriaQuery.select(criteriaQuery.from(entityClass));
		// 加入条件
		addRestrictions(criteriaQuery, filter);
		// TypedQuery执行查询与获取元模型实例
		TypedQuery<T> query = entityManager.createQuery(criteriaQuery)
				.setFlushMode(FlushModeType.COMMIT);
	...
	private void addRestrictions(CriteriaQuery<T> criteriaQuery,
								 Filter filter) {
		if (criteriaQuery==null || filter==null) {
			return;
		}
		Root<T> root = getRoot(criteriaQuery);
		// 实体的查询根对象
		...
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		// Predicate 过滤条件
		Order order = criteriaBuilder.desc(root.get(filter.getProperty()));
