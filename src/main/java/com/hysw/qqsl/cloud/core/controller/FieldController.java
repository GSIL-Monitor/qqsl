...
//    七参数验证接口
    @RequiresAuthentication
    @RequiresRoles(value = {"user:simple","account:simple"}, logical = Logical.OR)
    @RequestMapping(value = "/coordinate/parameter/seven/caculate", method = RequestMethod.POST)
    public @ResponseBody Message parameter7(@RequestBody  Map<String,Object> objectMap) {
        Message message = Message.parameterCheck(objectMap);
        if (message.getType() == Message.Type.FAIL) {
            return message;
        }
        ...
        Project project;
        try {
            project = projectService.find(Long.valueOf(projectId.toString()));
        } catch (Exception e) {
            return new Message(Message.Type.FAIL);
        }
        String central = coordinateService.getCoordinateBasedatum(project);
        ...
        Object commonPointStr = objectMap.get("commonPointStr");
        Object baseLevelType = objectMap.get("baseLevelType");
        if (commonPointStr == null || baseLevelType == null) {
            return new Message(Message.Type.FAIL);
        }
        JSONArray jsonArray = JSONArray.fromObject(commonPointStr);
        double[][] param = new double[3][3];
        double[][] param84 = new double[3][3];
        for (int i = 0; i < jsonArray.size(); i++) {
            JSONObject jsonObject = JSONObject.fromObject(jsonArray.get(i));
            JSONObject xyz = JSONObject.fromObject(jsonObject.get("xyz"));
            JSONObject blh = JSONObject.fromObject(jsonObject.get("blh"));
            param[i][0] = Double.valueOf(xyz.get("x").toString());
            param[i][1] = Double.valueOf(xyz.get("y").toString());
            param[i][2] = Double.valueOf(xyz.get("z").toString());
            param84[i][0] = Double.valueOf(blh.get("x").toString());
            param84[i][1] = Double.valueOf(blh.get("y").toString());
            param84[i][2] = Double.valueOf(blh.get("z").toString());
        }
        Matrix R;
        try {
            R = transFromService.calculate7Param(central,baseLevelType.toString(),param, param84);
        } catch (Exception e) {
            return new Message(Message.Type.FAIL);
        }
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("X", R.get(0, 0));
        jsonObject.put("Y", R.get(1, 0));
        jsonObject.put("Z", R.get(2, 0));
        jsonObject.put("K", R.get(3, 0));
        jsonObject.put("EX", R.get(4, 0));
        jsonObject.put("EY", R.get(5, 0));
        jsonObject.put("EZ", R.get(6, 0));
        return new Message(Message.Type.OK,jsonObject);
    }
//    四参数验证接口
    @RequiresAuthentication
    @RequiresRoles(value = {"user:simple","account:simple"}, logical = Logical.OR)
    @RequestMapping(value = "/coordinate/parameter/four/caculate", method = RequestMethod.POST)
    public @ResponseBody Message parameter4(@RequestBody  Map<String,Object> objectMap) {
        Message message = Message.parameterCheck(objectMap);
        if (message.getType() == Message.Type.FAIL) {
            return message;
        }
        Matrix R;
        try {
            R = transFromService.calculate4Param(param54, param84);
        } catch (Exception e) {
            return new Message(Message.Type.FAIL);
        }
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("X0", R.get(0, 0));
        jsonObject.put("Y0", R.get(1, 0));
        jsonObject.put("T", R.get(2, 0));
        jsonObject.put("K", R.get(3, 0));
        return new Message(Message.Type.OK,jsonObject);
    }
