...
import com.hysw.qqsl.cloud.dao.CoordinateDao;
...
public class CoordinateService extends BaseService<Coordinate, Long> {
	...
	public Message readExcels(InputStream is, String central, String s, Project project) throws IOException {
		Workbook wb;
		if (s.trim().toLowerCase().equals("xls")) {
			wb = new HSSFWorkbook(is);
			return readExcel(central, wb, project);
		} else if (s.trim().toLowerCase().equals("xlsx")) {
		...
		for (int rowNum = 0; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			if (row != null) {
				...
				if (type != null&&type.toString().trim()!= "") {
					for (int i = 0; i < CommonAttributes.BASETYPEC.length; i++) {
						graph.setBaseType(Config.CommonType.TSD);
						if (CommonAttributes.BASETYPEC[i]
								.equals(type.trim())) {
							graph.setBaseType(Config.CommonType.valueOf(CommonAttributes.BASETYPEE[i]));
							break;
						}
					}
				}
	...
	public String listToString(List<Graph> list, JSONObject coordinate) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if(list.get(i).getCoordinates().get(0).getLongitude().equals("0")){
				...
				baseType = Config.CommonType.GONGGXM;
				for (int i1 = 0; i1 < CommonAttributes.TYPELINEAREAC.length; i1++) {
					if (CommonAttributes.TYPELINEAREAC[i1].trim().equals(list.get(i).getCoordinates().get(0).getElevation().trim())) {
						baseType = Config.CommonType.valueOf(CommonAttributes.TYPELINEAREAE[i1]);
						break;
					}
				}
