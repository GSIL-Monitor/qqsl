...
import com.hysw.qqsl.cloud.entity.build.*;
...
//        List<Attribe> attribes1=attribeService.findByBuilds(builds1);
        ...
        for (int i = 0; i < coordinates.size(); i++) {
            ...
            graph.setBaseType(Config.CommonType.valueOf(type.toString()));
            ...
            if (attribes != null) {
                ...
                if (build == null) {
                    ...
                    build.setType(Config.CommonType.valueOf(type.toString()));
...
//        Coordinate.Type type = null;
        Config.CommonType baseType = null;
...
//            if(attribe.get)
    ...
    public Workbook writeExcel(Project project, Build.Source source) {
        ...
        Map<Config.CommonType,List<Build>>map=groupBuild(list);
    ...
    private void writeCoordinateToExcelDesign(JSONArray jsonArray1, Workbook wb) {
        Map<Config.CommonType, JSONArray> map = new LinkedHashMap<>();
        JSONObject jsonObject;
        for (Object o : jsonArray1) {
            jsonObject = JSONObject.fromObject(o);
            String baseType = jsonObject.get("baseType").toString();
            JSONArray jsonArray2 = map.get(Config.CommonType.valueOf(baseType));
            if (jsonArray2 == null) {
                jsonArray2 = new JSONArray();
                jsonArray2.add(jsonObject);
                map.put(Config.CommonType.valueOf(baseType),jsonArray2);
            }else{
                jsonArray2.add(jsonObject);
                map.put(Config.CommonType.valueOf(baseType),jsonArray2);
            }
     ...
     * @param wb
     */
    private void writeDesignCoordinateToExcel(Map<Config.CommonType, JSONArray> map, Workbook wb) {
        for (Map.Entry<Config.CommonType, JSONArray> entry : map.entrySet()) {
            Sheet sheet = null;
            Row row = null;
            Cell cell = null;
...
//                flag = true;
     ...
     * @return
     */
    protected Map<Config.CommonType, List<Build>> groupBuild(List<Build> list) {
