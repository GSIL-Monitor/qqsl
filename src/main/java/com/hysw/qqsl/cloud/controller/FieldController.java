...
public class FieldController {
    ...
    public @ResponseBody Message account(@RequestParam String token, @RequestParam String mac) {
        if (token.length() == 0) {
        ...
        if (applicationTokenService.decrypt(token)) {
            return positionService.randomPosition(mac.trim());
        }
        return new Message(Message.Type.FAIL);
    }
    ...
    public @ResponseBody Message timeout(@RequestBody  Map<String,String> objectMap) {
        Message message = Message.parameterCheck(objectMap);
        if (message.getType() == Message.Type.FAIL) {
            return message;
        }
